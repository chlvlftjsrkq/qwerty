...
def summarize_with_codex(
    ...
    prompt = " ".join(
        [
            ...
            "Never select a roundup, headline compilation, news digest, clipping, or article that combines several unrelated stories as a representative item. Select a concrete single-topic report instead.",
            "Every input article has already passed a representative-image check. Preserve that selected input article's no so its verified image, title, and source stay aligned.",
